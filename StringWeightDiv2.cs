...
public class StringWeightDiv2
{
    ...
    public int countMinimums(int L)
    {
        ...
        return (int)(res * (res1 == 0 ? 1 : res1) % mod);
    }

    private long C(int p1, int p2)
    {
        long res = 1;
        for (int i = 0; i < p2; i++)
        {
            res = (res * (p1 - i) / (i + 1)) % mod;
        }
        return res;
    }
    ...
    public static void Test()
    {
        try
        {
            eq(0, (new StringWeightDiv2()).countMinimums(1), 26);
            eq(1, (new StringWeightDiv2()).countMinimums(2), 650);
            eq(2, (new StringWeightDiv2()).countMinimums(50), 488801539);
        }
        catch (Exception exx)
        {
